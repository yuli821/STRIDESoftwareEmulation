...
For the per-mix HAL composite suites generated by
``scripts/gen_configs.py`` (one suite per workload mix), pass
``--mix <name>`` together with a ``_only`` suite, e.g.::

    python scripts/run_comparison.py --suite stateless_only --mix balanced
    python scripts/run_comparison.py --suite stateful_only  --mix user_example

This reads from ``configs/hal_<mix>/<suite>/*.yaml`` and writes outputs
under ``results/hal_<mix>/comparison_<suite>/``.
"""
...
def main() -> None:
    ...
    ap.add_argument("--out", default=None,
                    help="override output dir (default: "
                         "results/[hal_<mix>/]comparison_<suite>)")
    ap.add_argument("--configs", nargs="*", default=None,
                    help="explicit config files (overrides suite/dir)")
    ap.add_argument("--mix", default=None,
                    help=("HAL composite mix name (e.g. balanced, "
                          "user_example, web_heavy, cache_heavy, "
                          "hadoop_heavy). When set together with a "
                          "*_only suite, reads from "
                          "configs/hal_<mix>/<suite>/*.yaml and writes "
                          "to results/hal_<mix>/comparison_<suite>/."))
    ap.add_argument("--test", action="store_true",
                    help=("Use the TEST configs generated by "
                          "scripts/gen_test_configs.py: reads from "
                          "configs/test/hal_<mix>/<suite>/*.yaml and "
                          "writes to results/test/hal_<mix>/"
                          "comparison_<suite>/. Requires --mix."))
    ap.add_argument("--synthetic", action="store_true",
                    help=("Use the SYNTHETIC diagnostic configs "
                          "generated by scripts/gen_synthetic_configs.py: "
                          "reads from configs/synthetic/<suite>/*.yaml "
                          "and writes to "
                          "results/synthetic/comparison_<suite>/."))
    args = ap.parse_args()

    root = os.path.dirname(HERE)

    def _suite_paths(suite: str) -> tuple[str, str]:
        """Return (input dir, default output dir) honouring --mix,
        --test, and --synthetic."""
        if args.synthetic:
            in_dir = os.path.join(root, "configs", "synthetic", suite)
            out_dir = os.path.join(root, "results", "synthetic",
                                   f"comparison_{suite}")
            return in_dir, out_dir
        if args.test:
            if not args.mix:
                raise SystemExit("--test requires --mix <name>")
            in_dir = os.path.join(root, "configs", "test",
                                  f"hal_{args.mix}", suite)
            out_dir = os.path.join(root, "results", "test",
                                   f"hal_{args.mix}",
                                   f"comparison_{suite}")
        elif args.mix:
            in_dir = os.path.join(root, "configs", f"hal_{args.mix}", suite)
            out_dir = os.path.join(root, "results", f"hal_{args.mix}",
                                   f"comparison_{suite}")
        else:
            in_dir = os.path.join(root, "configs", suite)
            out_dir = os.path.join(root, "results", f"comparison_{suite}")
        return in_dir, out_dir
    ...
    if args.configs:
        ...
        dir_path, default_out = _suite_paths("stateless_only")
        files = sorted(glob.glob(os.path.join(dir_path, "*.yaml")))
        out = args.out or default_out
        plot_stateless = True
        plot_stateful = False
    elif args.suite == "stateful_only":
        dir_path, default_out = _suite_paths("stateful_only")
        files = sorted(glob.glob(os.path.join(dir_path, "*.yaml")))
        out = args.out or default_out
        plot_stateless = False
        plot_stateful = True
    elif args.suite == "stateless":
        dir_path, default_out = _suite_paths("stateless")
        files = sorted(glob.glob(os.path.join(dir_path, "*.yaml")))
        out = args.out or default_out
        plot_stateless = True
        plot_stateful = False
    elif args.suite == "stateful":
        dir_path, default_out = _suite_paths("stateful")
        files = sorted(glob.glob(os.path.join(dir_path, "*.yaml")))
        out = args.out or default_out
        plot_stateless = False
        plot_stateful = True
    ...
    for name, s in summaries.items():
        ...
        rows.append({
            ...
            "sl_mean_fair_P": sl.get("mean_fairness_P", 0.0),
            "sl_mean_fair_B": sl.get("mean_fairness_B", 0.0),
            ...
            "sf_mean_fair_P": sf.get("mean_fairness_P", 0.0),
            "sf_mean_fair_B": sf.get("mean_fairness_B", 0.0),
